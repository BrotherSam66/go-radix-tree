...
func Inputs() {
	...
	for {
		...
		fmt.Println("请输入payloadInt，按回车键(空按回车随机，-1退出)：")
		_, _ = fmt.Scanln(&payloadInt)
		_ = Insert(key, "", payloadInt)
		//if key == "-1" {
		//	return
		//}
...
// @payload 插入的载荷值
// @author https://github.com/BrotherSam66/
func Insert(key string, payload string, payloadInt int) (err error) {
	if payload == "" {
		payload = key
	}
	...
	// 从root开始查找附加的位置tempNode=找到的节点, subKey=key剩余的残串
	tempNode, subKey, err := Search(key)
	...
	*/
	...
	// [1]key=path，==》完美找到，替换、补充值
	if subKey == tempNode.Path {
		err = PayloadModify(tempNode, payload, payloadInt)
		return
	}

	// [2]path包含key，==》（回头在key末尾分叉）
	if len(subKey) < len(tempNode.Path) && strings.Index(tempNode.Path, subKey) == 0 {
		_ = SplitOldNode(tempNode, subKey, payload, payloadInt)
		return
	}

	// [3]path互不包含key，==》（回头在两个不同点分叉），上半截造一个全空纯粹分支节点，两个尾巴做两个叶子节点
	if strings.Index(tempNode.Path, subKey) != 0 && strings.Index(subKey, tempNode.Path) != 0 { // 互相不是子串的头
		_ = Split3Node(tempNode, subKey, payload, payloadInt)
		return
	}
...
func PayloadModify(tempNode *radixmodels.RadixNode, payload string, payloadInt int) (err error) {
	if !IsIntInSlice(tempNode.PayloadIntSlice, payloadInt) {
		tempNode.PayloadIntSlice = append(tempNode.PayloadIntSlice, payloadInt)
		QuickSort(0, len(tempNode.PayloadIntSlice)-1, &tempNode.PayloadIntSlice)
	}
...
func IsIntInSlice(intSlice []int, inInt int) bool {
	left := 0
